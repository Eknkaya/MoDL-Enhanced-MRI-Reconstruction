...
# =========================================================================
# IMPROVEMENT (PROJE MODİFİKASYONU)
# Eski Sadece L2 (MSE) loss yerine, L1, L2 ve SSIM kompozit kayıp fonksiyonu eklendi.
# Amaç: Yüksek frekanslı anatomik detayları ve keskin kenarları korumak.
# =========================================================================

# 1. L2 Kaybı (Standart Ortalama Kare Hatası - Genel enerji farkını minimize eder)
loss_l2 = tf.reduce_mean(tf.reduce_sum(tf.pow(predT - orgT, 2), axis=0))

# 2. L1 Kaybı (Mutlak Fark - L2'nin yaptığı bulanıklaştırmayı engeller, daha keskin hatlar üretir)
loss_l1 = tf.reduce_mean(tf.reduce_sum(tf.abs(predT - orgT), axis=0))

# 3. SSIM Kaybı (Yapısal Benzerlik - İnsan gözünün algıladığı doku kalitesine odaklanır)
# max_val değerini MR verisinin maksimum-minimum aralığına göre dinamik hesaplıyoruz.
max_val = tf.reduce_max(orgT) - tf.reduce_min(orgT)
ssim_index = tf.reduce_mean(tf.image.ssim(predT, orgT, max_val=max_val))
loss_ssim = 1.0 - ssim_index 

# Toplam Kayıp: Hiperparametrelerle (0.5, 0.1, 0.4) ağırlıklandırılmış kompozit fonksiyon
loss = (0.5 * loss_l2) + (0.1 * loss_l1) + (0.4 * loss_ssim)
...
with tf.name_scope('optimizer'):
        # 1. Adım Sayacı (Optimizasyonun neresinde olduğumuzu takip etmek için)
        global_step = tf.Variable(0, trainable=False, name='global_step')
        
        # 2. Üstel Azalan Öğrenme Oranı Planlayıcısı (Exponential Decay)
        learning_rate = tf.train.exponential_decay(
            learning_rate=0.001,      # Başlangıç hızı (ADAM varsayılanı)
            global_step=global_step,  # Mevcut adım
            decay_steps=360,          # Her 360 adımda bir (Yani her 1 tam epoch'ta)
            decay_rate=0.90,          # Öğrenme oranını %10 azalt (0.90 ile çarp)
            staircase=True            # Pürüzsüz değil, basamak basamak (merdiven tipi) düşür
        )
        
        # 3. Dinamik öğrenme oranını ADAM optimizatörüne bağlama
        optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
        
        # 4. Gradyan hesaplama ve patlayan gradyanları kırpma (Gradient Clipping)
        gvs = optimizer.compute_gradients(loss)
        capped_gvs = [(tf.clip_by_value(grad, -1., 1.), var) for grad, var in gvs]
        
        # 5. Ağırlıkları güncelle ve sayaçları (global_step) 1 artır
